...
// New HTTP Methods with token
    ...
    private func sendPostRequest(token:String? = nil, url: String, body: String, completionHandler: @escaping (Data?, URLResponse?, Error?) -> Void) {
        var request = URLRequest(url: URL(string: url)!)
        request.setValue("application/json; charset=utf-8", forHTTPHeaderField: "Content-Type")
    ...
    private func sendGetRequest(token:String? = nil, url: String, parameters: [String: AnyObject], completionHandler: @escaping (Data?, URLResponse?, Error?) -> Void) {
    ...
    private func sendPutRequest(token:String? = nil, url: String, parameters: [String: AnyObject], completionHandler: @escaping (Data?, URLResponse?, Error?) -> Void) {
        let parameterString = parameters.stringFromHttpParameters()
        var request = URLRequest(url: URL(string: url)!)
        request.setValue("application/json; charset=utf-8", forHTTPHeaderField: "Content-Type")
    ...
    private func sendPutRequest(token:String? = nil, url: String, body: String, completionHandler: @escaping (Data?, URLResponse?, Error?) -> Void) {
        var request = URLRequest(url: URL(string: url)!)
        request.httpBody = body.data(using: .utf8)
        request.httpMethod = "PUT"
        request.setValue("application/json; charset=utf-8", forHTTPHeaderField: "Content-Type")
    ...
    private func sendDeleteRequest(token:String? = nil, url:String, body:String, completionHandler: @escaping (Data?, URLResponse?, Error?) -> Void) {
        var request = URLRequest(url: URL(string: url)!)
        request.httpBody = body.data(using: .utf8)
    ...
    private func getAuthoriztion(token:String?) -> String? {
        if token == nil || token! == "" {
            return nil
        }
    ...
    private func getSecurityProtocol(wifi:Network) -> String {
